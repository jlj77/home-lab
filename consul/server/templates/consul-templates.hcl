...
#  token = "abcd1234"
  ...
  ssl {
    ...
    cert = "/opt/consul_/tls/client.consul-s0.dc-1.consul.crt"
    key = "/opt/consul_/tls/client.consul-s0.dc-1.consul.key"
...
#    ca_cert = ""
    ...
    server_name = "server.dc-1.consul"
  }
}
...
template {
  ...
  # the `contents` option.
  source = "/opt/consul_/templates/gossip.key.tpl"

  # This is the destination path on disk where the source template will render.
  # If the parent directories do not exist, Consul Template will attempt to
  # create them, unless create_dest_dirs is false.
  destination = "/opt/consul_/gossip/gossip.key"
...
#  contents = "{{ keyOrDefault \"service/redis/maxconns@east-aws\" \"5\" }}"
  ...
  command = "/usr/local/sbin/rotate_gossip_key.sh"
